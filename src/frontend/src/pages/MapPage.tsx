...
// Positions on the SVG map [x%, y%] for portrait layout
const NODE_POSITIONS = [
  { x: 82, y: 12 }, // Kolkata (India, upper right)
  { x: 68, y: 22 }, // Dubai (Middle East)
  { x: 35, y: 42 }, // Atlantic (mid ocean)
  { x: 25, y: 50 }, // Night Flight (mid-Atlantic)
  { x: 18, y: 66 }, // Emergency (Caribbean)
  { x: 15, y: 78 }, // Panama City (Central America)
];
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 text-center">
        ...
        <p className="font-mono text-xs mt-1" style={{ color: "#00FFFF88" }}>
          Carol's Journey — Kolkata to Panama City
        </p>
      </div>

      {/* Relationship bond meter */}
      <div className="relative z-20 flex items-center gap-2 px-2">
        <span className="font-mono text-xs" style={{ color: "#FF149388" }}>
          BOND
        </span>
        <div
      ...
      >
        ...
        >
          {/* Background continents — simplified pixel blobs */}
          {/* Europe/Africa blob */}
          <path
          ...
          {/* Flight path lines between nodes */}
          {NODE_POSITIONS.slice(0, -1).map((pos, idx) => {
            ...
            return (
              <line
                key={`seg-${LEVELS[idx].id}-${LEVELS[idx + 1].id}`}
                x1={pos.x}
                y1={pos.y}
                x2={next.x}
                y2={next.y}
            ...
            return (
              <g key={level.id} transform={`translate(${pos.x}, ${pos.y})`}>
                ...
                <circle
                  r="4"
                  fill={
                    isCompleted ? color : isUnlocked ? "#0d1135" : "#0a0e27"
                  }
      ...
      {/* Level list — scrollable detail cards */}
            ...
            <button
              ...
              style={{
                ...
                background: isUnlocked
                  ? "linear-gradient(135deg, rgba(10,14,39,0.98), rgba(10,14,39,0.8))"
                  : "rgba(10,14,39,0.4)",
                opacity: isUnlocked ? 1 : 0.4,
            ...
            >
              ...
              <div className="flex-shrink-0 font-mono text-xs font-bold">
                {isCompleted ? (
                  ...
                  <span className="animate-pulse" style={{ color: "#00FFFF" }}>
                    ► GO
                  </span>
                ) : !isUnlocked ? (
                  <span style={{ color: "#333366" }}>LOCK</span>
                ) : null}
              </div>
            </button>
          );
